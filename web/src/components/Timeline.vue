...
<template>
  <div class="relative">
    ...
    <!-- Empty State -->
    <div v-else-if="items.length === 0" class="text-center py-8 text-gray-500 bg-gray-50 rounded-lg border border-gray-100 border-dashed">
      {{ emptyText }}
    </div>
    ...
    <ul v-else class="space-y-0">
      <li v-for="(item, index) in items" :key="index" class="relative flex gap-4 group pb-8 last:pb-0">
        ...
        <div
          ...
          class="absolute top-8 left-4 w-0.5 h-full -ml-px bg-gray-200"
          aria-hidden="true"
        ></div>

        <!-- Icon/Dot -->
        <div
          class="relative flex h-8 w-8 items-center justify-center rounded-full border ring-4 ring-white shrink-0 z-10"
        ...
        <!-- Content -->
        <div class="flex-1 min-w-0 pt-0.5 cursor-default group-hover:bg-gray-50/50 rounded -ml-2 pl-2 -mr-2 pr-2 transition-colors duration-200" @click="onClick(item)">
          <div class="flex justify-between items-start text-sm">
            <h3 class="font-medium text-gray-900">{{ item.title }}</h3>
            <time v-if="item.date" class="text-xs text-gray-500 whitespace-nowrap pl-4 pt-0.5">{{ item.date }}</time>
          </div>
