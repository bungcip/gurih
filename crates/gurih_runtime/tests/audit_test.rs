...
async fn test_audit_trail() {
    ...
    let logs = datastore
        .query("SELECT * FROM \"_audit_log\" WHERE entity = 'TestEntity'")
        .await
        .expect("Query failed");
    ...
    let logs = datastore
        .query("SELECT * FROM \"_audit_log\" WHERE entity = 'TestEntity' ORDER BY timestamp ASC")
        .await
        .expect("Query failed");
    ...
    let logs = datastore
        .query("SELECT * FROM \"_audit_log\" WHERE entity = 'NoTrackEntity'")
        .await
        .expect("Query failed");
