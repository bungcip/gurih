use gurih_ir::{
    EntitySchema, FieldSchema, FieldType, FormItem, FormSchema, FormSection, GridDef, Ownership, RelationshipSchema,
...
fn test_form_grid_columns_respected() {
    ...
    let parent_entity = EntitySchema {
        ...
        relationships: vec![RelationshipSchema {
            ...
            target_entity: Symbol::from("OrderItem"),
    ...
    let form = FormSchema {
        ...
        sections: vec![FormSection {
            ...
            items: vec![FormItem::Grid(GridDef {
    ...
    let result = engine
        .generate_ui_schema(&schema, form_name)
        .expect("Failed to generate UI schema");
    ...
    if col_names.contains(&"price") {
        panic!(
            "Grid contains 'price' but it should be excluded! Columns found: {:?}",
            col_names
        );
    }
}
