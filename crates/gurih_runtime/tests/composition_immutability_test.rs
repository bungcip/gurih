use gurih_ir::{
    EntitySchema, FieldSchema, FieldType, Schema, StateSchema as WorkflowState, Symbol, RelationshipSchema, RelationshipType,
    WorkflowSchema, Transition, Ownership,
};
...
async fn test_child_update_fails_if_parent_locked() {
    ...
    let parent_id = engine.create("Parent", json!({ "status": "Draft" }), &ctx).await.unwrap();

    // 3. Create Child
    let child_id = engine.create("Child", json!({
        "parent_id": parent_id,
        "amount": "100.00"
    }), &ctx).await.unwrap();

    // 4. Update Parent to Posted (Immutable)
    engine.update("Parent", &parent_id, json!({ "status": "Posted" }), &ctx).await.unwrap();

    // 5. Try to Update Child
    let result = engine.update("Child", &child_id, json!({ "amount": "999.00" }), &ctx).await;
...
async fn test_create_child_into_locked_parent() {
    ...
    let result = engine.create("Child", json!({
        "parent_id": parent_id,
        "amount": "100.00"
    }), &ctx).await;
...
async fn test_move_child_to_locked_parent() {
    ...
    let p1_id = engine.create("Parent", json!({ "status": "Draft" }), &ctx).await.unwrap();
    engine.update("Parent", &p1_id, json!({ "status": "Posted" }), &ctx).await.unwrap();

    // 3. Create Parent 2 (Draft)
    let p2_id = engine.create("Parent", json!({ "status": "Draft" }), &ctx).await.unwrap();
    ...
    let result = engine.update("Child", &child_id, json!({ "parent_id": p1_id }), &ctx).await;
