...
#[tokio::test]
async fn test_finance_reverse_journal_missing_entry_number() {
    // 1. Setup Schema
    let mut entities = HashMap::new();
    ...
    // 4. Create Data (Missing entry_number)
    // Journal
    let je_id = data_engine
        .create(
            "JournalEntry",
            json!({
                "id": "je-2",
                // NO entry_number here
                "description": "Original Entry 2",
                "status": "Posted"
            }),
            &ctx,
        )
    ...
    if let Err(e) = res {
        assert!(e.to_string().contains("Original journal entry missing entry_number"));
    }
}
