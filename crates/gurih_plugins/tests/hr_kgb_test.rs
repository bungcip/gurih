...
async fn test_kgb_eligibility_success() {
    ...
    let two_years_ago = (Local::now().date_naive() - chrono::Duration::days(366 * 2 + 10)).format("%Y-%m-%d").to_string();
    ...
    let peg_id = data_engine.create("Pegawai", peg_payload, &ctx).await.expect("Create Pegawai failed");

    // 2. Create SKP records
    let current_year = Local::now().year();

    data_engine.create("RiwayatSKP", json!({
        "pegawai": peg_id,
        "tahun": current_year - 1,
        "predikat": "Baik"
    }), &ctx).await.expect("Create SKP 1 failed");

    data_engine.create("RiwayatSKP", json!({
        "pegawai": peg_id,
        "tahun": current_year - 2,
        "predikat": "Sangat Baik"
    }), &ctx).await.expect("Create SKP 2 failed");
    ...
    let sub_id = data_engine.create("PengajuanKGB", sub_payload, &ctx).await.expect("Create Submission failed");

    // 4. Attempt Transition
    let update_payload = json!({ "status": "Diajukan" });
    data_engine.update("PengajuanKGB", &sub_id, update_payload, &ctx).await.expect("Transition failed but should succeed");
}
...
async fn test_kgb_eligibility_fail_time() {
    ...
    let peg_id = data_engine.create("Pegawai", json!({"tmt_golongan": recent}), &ctx).await.expect("Create Peg failed");

    // Add valid SKP
    let current_year = Local::now().year();
    data_engine.create("RiwayatSKP", json!({"pegawai": peg_id, "tahun": current_year - 1, "predikat": "Baik"}), &ctx).await.unwrap();
    data_engine.create("RiwayatSKP", json!({"pegawai": peg_id, "tahun": current_year - 2, "predikat": "Baik"}), &ctx).await.unwrap();

    let sub_id = data_engine.create("PengajuanKGB", json!({"pegawai": peg_id, "status": "Draft"}), &ctx).await.unwrap();

    let res = data_engine.update("PengajuanKGB", &sub_id, json!({"status": "Diajukan"}), &ctx).await;
    assert!(res.is_err());
...
async fn test_kgb_eligibility_fail_skp() {
    ...
    // Old TMT (Valid)
    let old = (Local::now().date_naive() - chrono::Duration::days(366 * 3)).format("%Y-%m-%d").to_string();
    let peg_id = data_engine.create("Pegawai", json!({"tmt_golongan": old}), &ctx).await.expect("Create Peg failed");

    // Invalid SKP (Cukup)
    let current_year = Local::now().year();
    data_engine.create("RiwayatSKP", json!({"pegawai": peg_id, "tahun": current_year - 1, "predikat": "Baik"}), &ctx).await.unwrap();
    data_engine.create("RiwayatSKP", json!({"pegawai": peg_id, "tahun": current_year - 2, "predikat": "Cukup"}), &ctx).await.unwrap();
