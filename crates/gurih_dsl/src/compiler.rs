...
pub fn compile(src: &str) -> Result<Schema, CompileError> {
    ...
    let process_entity = |entity_def: &ast::EntityDef, module_name: Option<&str>| -> Result<EntitySchema, CompileError> {
        let mut fields = vec![];
        ...
        let relationships = entity_def.relationships.iter().map(|r| RelationshipSchema {
            name: r.name.clone(),
        ...
        if entity_def.options.is_submittable { options.insert("is_submittable".to_string(), "true".to_string()); }
        if entity_def.options.track_changes { options.insert("track_changes".to_string(), "true".to_string()); }
        if entity_def.options.is_single { options.insert("is_single".to_string(), "true".to_string()); }
    ...
    for wf_def in &ast_root.workflows {
        ...
        ir_workflows.insert(wf_def.name.clone(), WorkflowSchema {
            ...
            initial_state: wf_def.states.iter().find(|s| s.initial).map(|s| s.name.clone()).unwrap_or_default(),
    ...
    for layout_def in &ast_root.layouts {
        ...
        let mut props = header_props; // Merge props? Simple flattening for now
        
        ir_layouts.insert(layout_def.name.clone(), LayoutSchema {
    ...
    for menu_def in &ast_root.menus {
        ir_menus.insert(menu_def.name.clone(), MenuSchema {
    ...
    for page_def in &ast_root.pages {
        let content = match &page_def.content {
            ast::PageContent::Datatable(dt) => PageContentSchema::Datatable(DatatableSchema {
                ...
                columns: dt.columns.iter().map(|c| DatatableColumnSchema {
                    field: c.field.clone(),
                    label: c.label.clone(),
                }).collect(),
                actions: dt.actions.iter().map(|a| ActionSchema {
                    label: a.label.clone(),
                    to: a.to.clone(),
                    icon: a.icon.clone(),
                }).collect(),
        ...
        ir_pages.insert(page_def.name.clone(), PageSchema {
    ...
    for db_def in &ast_root.dashboards {
        ir_dashboards.insert(db_def.name.clone(), DashboardSchema {
            ...
            widgets: db_def.widgets.iter().map(|w| WidgetSchema {
                name: w.name.clone(),
