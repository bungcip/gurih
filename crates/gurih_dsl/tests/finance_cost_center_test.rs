...
use gurih_ir::{Symbol, FieldType, RelationshipType};
use std::path::Path;
...
fn test_finance_cost_center_support() {
    ...
    let cost_center = schema.entities.get(&cost_center_sym).expect("CostCenter entity not found");

    // Check fields
    let code_field = cost_center.fields.iter().find(|f| f.name == Symbol::from("code")).unwrap();
    assert!(matches!(code_field.field_type, FieldType::String));

    let name_field = cost_center.fields.iter().find(|f| f.name == Symbol::from("name")).unwrap();
    ...
    let journal_line = schema.entities.get(&journal_line_sym).expect("JournalLine entity not found");

    let cc_rel = journal_line.relationships.iter().find(|r| r.target_entity == Symbol::from("CostCenter"));
    ...
    let query = schema.queries.get(&query_sym).expect("IncomeStatementByCostCenterQuery not found");
    ...
    // Check children
    let report_route = finance_group.children.iter().find(|r| r.path == "/reports/income-statement-by-cost-center");
