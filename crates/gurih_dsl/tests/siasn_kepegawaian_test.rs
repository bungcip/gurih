...
fn test_siasn_kepegawaian_parsing() {
    ...
    let entity = ast.modules.iter()
        .flat_map(|m| &m.entities)
        .find(|e| e.name == "RiwayatHukumanDisiplin")
        .expect("RiwayatHukumanDisiplin entity not found");

    // Check fields
    assert!(entity.fields.iter().any(|f| f.name == "tingkat_hukuman"), "tingkat_hukuman field missing");
    assert!(entity.fields.iter().any(|f| f.name == "jenis_hukuman"), "jenis_hukuman field missing");
    assert!(entity.fields.iter().any(|f| f.name == "nomor_sk"), "nomor_sk field missing");
}
