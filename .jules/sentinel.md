...
## 2026-04-10 - [HIGH] CPU Exhaustion DoS via Unbounded Inputs in PBKDF2
**Vulnerability:** The `login` function in `AuthEngine` processed unbounded `username` and `password` strings through the PBKDF2 hashing algorithm. An attacker could send massive strings (e.g., several megabytes), forcing the server to spend excessive CPU time hashing them, leading to a Denial of Service (DoS) due to CPU exhaustion.
**Learning:** Cryptographic operations like PBKDF2 are intentionally slow. If inputs to these functions are not strictly bounded in length *before* processing, they become easy targets for asymmetric DoS attacks where a small request causes massive server load.
**Prevention:** Enforced strict maximum length limits on `username` (255 bytes) and `password` (1024 bytes) at the very beginning of the authentication flow, failing fast if the limits are exceeded.
