...
## 2024-06-10 - [CRITICAL] CPU Exhaustion DoS via Rate Limiting Dummy Hash
**Vulnerability:** The rate limiting block in `AuthEngine::login` performed a dummy PBKDF2 hash computation (`verify_password("dummy", &self.dummy_hash)`) before returning a "Too many failed attempts" error. This was originally intended to prevent timing side-channels for user enumeration. However, because an attacker could intentionally trigger rate limiting with invalid attempts and then flood the endpoint, forcing the server to continuously perform expensive PBKDF2 calculations, this created a CPU exhaustion Denial of Service (DoS) vulnerability.
**Learning:** Performing expensive dummy cryptographic operations during rate limit rejections unnecessarily consumes server resources and converts an authentication attempt into a DoS vector. Since rate limiting explicitly applies to any username that has exceeded the threshold (regardless of whether the user exists or not), returning immediately upon a rate limit does not introduce a user enumeration timing side-channel. The response time applies equally to both valid and invalid locked-out usernames.
**Prevention:** Removed the dummy hash computation from the rate-limiting block in `gurih_runtime::src::auth.rs`, ensuring that rate-limited requests fail fast and securely without exhausting server CPU resources.
