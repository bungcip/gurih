...
## 2024-06-10 - [HIGH] Unbounded Memory Growth in AuthEngine (User Enumeration DoS)
**Vulnerability:** `login_attempts` in `AuthEngine` grew indefinitely if the input username was excessively long. The previous length check was outside the scope of where rate limits were checked, meaning an attacker could bypass the length checks and OOM the server by supplying arbitrarily long usernames.
**Learning:** Checking inputs for maximum length to prevent CPU exhaustion (like for PBKDF2) is crucial, but failing to apply those same bounds to inputs that are cached in memory (like in `HashMap`s for rate limiting) creates a secondary memory exhaustion (OOM) Denial of Service vulnerability.
**Prevention:** Ensured that excessively long inputs (e.g., usernames > 255 bytes) are not tracked in the `login_attempts` memory map. We only track rate limiting for reasonably sized inputs, mitigating the memory exhaustion risk.
