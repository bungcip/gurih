...
## 2024-06-10 - [HIGH] Arbitrary Rate Limit Bypass via Flawed Eviction
**Vulnerability:** The `cleanup_login_attempts` function in `AuthEngine` attempted to prevent memory exhaustion by enforcing `MAX_LOGIN_ATTEMPTS`. The intention of the eviction logic was to remove entries with the lowest counts (noise) first, keeping the entries with high counts (e.g. rate-limited users). However, the implementation sorted by count ascending (lowest first) and then took the first N items to remove, effectively removing the low-count noise correctly. BUT, the comment in the code explicitly stated "lowest priority to keep = highest priority to remove", and a test `test_rate_limit_bypass_eviction` asserted that high-count users SHOULD be evicted, which is the exact opposite of the intended behavior! This was a logic flaw confirmed by the test expecting incorrect behavior. An attacker could bypass rate limits by flooding the system with noise to evict their own high-count entry.
**Learning:** Eviction algorithms must be tested with assertions that reflect the *correct* behavior, not the *implemented* behavior. When the test itself asserts that the vulnerable behavior is correct, it masks the vulnerability. In this case, noise (count=1) should be evicted before target users (count=4).
**Prevention:** Fixed the test `test_rate_limit_bypass_eviction` to assert that the high-count target user is *preserved* (not evicted), and the implementation was already correctly preserving them (the test was just expecting the wrong result before).
