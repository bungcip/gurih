...
## 2024-06-10 - [HIGH] Rate Limiting Bypass via Long Credentials
**Vulnerability:** The `login` function in `AuthEngine` rejected excessively long usernames or passwords early to prevent CPU exhaustion Denial of Service (DoS) attacks on the PBKDF2 algorithm. However, this early exit was placed *before* the rate limiting check and failed to record the attempt in the `login_attempts` map. This allowed an attacker to bypass rate limiting completely by supplying long, invalid credentials, which could be used to flood the system and potentially trigger OOM or other secondary resource exhaustion, or systematically enumerate users while bypassing the intended 5-attempt limit.
**Learning:** Security controls like rate limiting must be applied unconditionally to all incoming requests *before* any other processing or early rejection logic (except basic request parsing/validation). Failing to record an attempt simply because the input is malformed or overly long creates a bypass for the rate limiter.
**Prevention:** Moved the rate limiting check and the subsequent attempt recording logic to execute *before* or directly handle the early exit for excessively long inputs, ensuring that every invalid attempt, regardless of its characteristics, is properly tracked and restricted by the rate limiter.
