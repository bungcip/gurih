...
## 2024-06-03 - [CRITICAL] SQL Injection via Schema Definition in Formula and Selection Aliases (Self-SQLi)
**Vulnerability:** The `QueryEngine` trusted formula names and selection aliases from the schema to be safe, directly interpolating them into SQL `SELECT` statements (`... AS "alias"` or `... AS formula_name`). A malicious schema could execute arbitrary SQL by including statements within these identifiers.
**Learning:** All identifiers derived from schema definitions must be treated as untrusted input and validated before being used in raw SQL construction. "Internal" names and aliases are never safe if they originate from user-provided configuration.
**Prevention:** Implemented mandatory identifier validation (`crate::store::validate_identifier`) in `QueryEngine` for all formula names, selection aliases, and hierarchy rollup fields before executing the SQL query, rejecting any non-alphanumeric identifiers and mitigating the Self-SQLi vulnerability.
