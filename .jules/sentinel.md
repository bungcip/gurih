...
## 2026-04-09 - Prevent CPU Exhaustion DoS in PBKDF2 Hashing
**Vulnerability:** The `login` function in `auth.rs` did not enforce maximum length limits on `username` and `password` inputs before processing them. An attacker could provide extremely long strings (e.g., millions of characters), causing the computationally expensive PBKDF2 hashing algorithm to consume excessive CPU resources and potentially leading to a Denial of Service (DoS).
**Learning:** Security algorithms that are intentionally slow (like PBKDF2, bcrypt, or Argon2) must be protected from unbounded input lengths. If input lengths are not strictly limited prior to hashing, the intended computational cost (work factor) intended to slow down brute-force attacks is instead weaponized to exhaust server resources.
**Prevention:** Enforce strict maximum length limits on `username` (e.g., 255 bytes) and `password` (e.g., 1024 bytes) inputs at the earliest possible stage in authentication functions prior to processing.
