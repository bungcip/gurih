...
## 2024-06-02 - [CRITICAL] SQL Injection via Formula Alias (Self-SQLi)
**Vulnerability:** The `QueryEngine` trusted formula names from the schema to be safe, directly interpolating them into SQL SELECT aliases without quotes or validation. A malicious schema could inject arbitrary SQL syntax.
**Learning:** In DSL/Schema-driven architectures, all schema-provided identifiers, including abstract concepts like "formula names", must be treated as untrusted input. Validation and quoting must be applied universally.
**Prevention:** Added double-quotes to all formula aliases (`AS "{}"`) and enforced `validate_identifier` on formula names during query generation.
