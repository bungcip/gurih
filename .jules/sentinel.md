...
## 2024-06-10 - [CRITICAL] CPU Exhaustion DoS via Rate Limiter Timing Mitigation
**Vulnerability:** A previous security fix (2026-04-25) added a dummy PBKDF2 hash computation (`verify_password("dummy", &self.dummy_hash)`) inside the rate-limiting rejection block to mitigate a timing attack that allowed user enumeration. However, this converted a minor information leak into a critical CPU exhaustion Denial of Service (DoS) vulnerability. An attacker could intentionally trigger rate limits and then flood the endpoint, forcing the server to perform expensive cryptographic operations (600,000 PBKDF2 iterations) for every single rate-limited request, easily exhausting server CPU resources.
**Learning:** Security mitigations must not introduce more severe vulnerabilities. While timing side-channels are a concern during the actual authentication process (where valid vs invalid passwords or usernames could be enumerated), returning a rate limit error immediately applies equally to both valid and invalid usernames that are locked out. Therefore, returning early on a rate limit does not introduce a user enumeration side-channel, as the response is constant-time for all rate-limited inputs. Performing expensive operations during an explicit rejection path is a textbook DoS vector.
**Prevention:** Removed the dummy hash computation from the rate limiting block. Rate limited requests now return immediately, saving CPU cycles without re-introducing the original user enumeration timing attack.
